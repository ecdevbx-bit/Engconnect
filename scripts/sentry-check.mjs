...
async function waitForIssue(query, label) {
  for (let i = 0; i < 24; i++) {
    const issues = await sentry(`/projects/${ORG}/${PROJECT}/issues/?query=${encodeURIComponent(query)}&statsPeriod=1h`);
    if (Array.isArray(issues) && issues.length) return issues[0];
    await new Promise((r) => setTimeout(r, 5000));
  }
  console.log(`   (no ${label} issue after 2 min)`);
  return null;
}
...
if (body.traceId) {
  const issue = await waitForIssue(`traceId:${body.traceId}`, "server");
  check(!!issue, "server error reached Sentry", issue ? issue.permalink : "");
}
...
const t = await fetch(`${APP}/monitoring?o=${orgId}&p=${projectId}`, { method: "POST", body: envelope, headers: { "Content-Type": "text/plain;charset=UTF-8" } });
check(t.status < 300, "the /monitoring tunnel accepted a browser event", String(t.status));
if (t.status < 300) {
  const issue = await waitForIssue(`check:${marker}`, "tunnel");
  check(!!issue, "tunnel event reached Sentry", issue ? issue.permalink : "");
}
